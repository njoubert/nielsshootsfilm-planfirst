...
#   - frontend/scripts/
#   - scripts/
#   - Root convenience scripts: dev.sh, format.sh, provision.sh
...
for file in "$@"; do
    case "$file" in
        backend/scripts/*|frontend/scripts/*|scripts/*|dev.sh|format.sh|provision.sh)
            # Allowed location
            ;;
        *)
            ...
            echo "     • scripts/"
            echo "     • Root convenience scripts: dev.sh, format.sh, provision.sh"
            EXIT_CODE=1
            ;;
    esac
done
